...
class Post extends Component {
	...
	render() {
		...
		var showPostsListSidebar = this.props.posts.map((ele, index) => {
			return <PostItemSidebar post={ele} key={index} index={index + 1} />;
		});
					...
					<div className="container">
						<div className="row">
							<div class="col-lg-8">
								<p style={{ padding: 0 }}>
									<span
										style={{ fontWeight: 800, fontSize: '50px', color: 'black' }}
										className="mb-3"
									>
										{post.title}
									</span>
								...
								<p>{ReactHtmlParser(this.editImageInContent(post.content))}</p>
							</div>
